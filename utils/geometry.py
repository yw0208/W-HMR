...
from core.constants import FOCAL_LENGTH
...
def convert_pare_to_full_img_cam(
        ...
        img_w, img_h, focal_length=None,Tz=None):
...
def unbiased_gram_schmidt(x):
    k=x.shape[1]
    x = x.reshape(-1, 3, 3)
    t1 = x[:, :, 0]
    t2 = x[:, :, 1]
    t3 = x[:, :, 2]
    r1 = F.normalize((torch.cross(t2, t3) + t1) / 2.)
    r2_ = (torch.cross(t3, r1) + t2) / 2.
    r2 = F.normalize(r2_ - (torch.einsum('bi,bi->b', r2_, r1).unsqueeze(-1) * r1))
    r3 = torch.cross(r1, r2)
    rotmat = torch.stack((r1, r2, r3), dim=-1)
    rotmat = rotmat.reshape(-1, k, 3, 3)
    return rotmat
...
def projection(pred_joints, pred_camera, retain_z=False):
    pred_cam_t = torch.stack([pred_camera[:, 1],
                              pred_camera[:, 2],
                              2 * FOCAL_LENGTH / (cfg.IMG_RES.HEIGHT * pred_camera[:, 0] + 1e-9)], dim=-1)
    ...
    pred_keypoints_2d = pred_keypoints_2d / (
            torch.tensor([cfg.IMG_RES.WIDTH, cfg.IMG_RES.HEIGHT], device=pred_keypoints_2d.device) / 2.)
    # pred_keypoints_2d=(pred_keypoints_2d+1)*(
    #         torch.tensor([cfg.IMG_RES.WIDTH, cfg.IMG_RES.HEIGHT], device=pred_keypoints_2d.device) / 2.)
...
def perspective_projection(points, rotation, translation,
    ...
    projected_points = torch.einsum('bij,bkj->bki', K, projected_points.to(torch.float32))
...
def estimate_translation(S, joints_2d, focal_length=5000., img_size=[224., 224.]):
    """Find camera translation that brings 3D joints S closest to 2D the corresponding joints_2d.
    Input:
