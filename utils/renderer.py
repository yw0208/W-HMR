...
import trimesh
import numpy as np
from pare.utils.vis_utils import get_smpl_faces
...
class IUV_Renderer(object):
    ...
    def __init__(self, focal_length=constants.FOCAL_LENGTH, orig_size=(224, 224), output_size=(56, 56), mode='iuv',
                 device=torch.device('cuda'), mesh_type='smpl'):
        ...
        K = np.array([[self.focal_length, 0., self.orig_size[1] / 2.],
                      [0., self.focal_length, self.orig_size[0] / 2.],
                      [0., 0., 1.]])
        ...
        if self.orig_size[0] != 224:
            rander_scale_w = self.orig_size[1] / float(224)
            rander_scale_h = self.orig_size[0] / float(224)
            K[0, 0] *= rander_scale_w
            K[1, 1] *= rander_scale_h
            K[0, 2] *= rander_scale_w
            K[1, 2] *= rander_scale_h
    ...
    def camera_matrix(self, cam):
        ...
        t = torch.stack([-cam[:, 1], -cam[:, 2], 2 * self.focal_length / (self.orig_size[0] * cam[:, 0] + 1e-9)], dim=-1)
    ...
    def verts2iuvimg(self, verts, cam, iwp_mode=True):
        ...
        cameras = PerspectiveCameras(device=verts.device, R=R, T=t, K=K, in_ndc=False,
                                     image_size=[self.orig_size])

        iuv_image = self.renderer(mesh, cameras=cameras)
        iuv_image = iuv_image[..., :3].permute(0, 3, 1, 2)
        return iuv_image

class Depth_Renderer(object):
    def __init__(self, focal_length=constants.FOCAL_LENGTH, orig_size=(224, 224), output_size=(56, 56), mode='depth', device=torch.device('cuda'),
                 mesh_type='smpl'):
    ...
    def verts2depthimg(self, verts, cam, iwp_mode=True):
        batch_size = verts.size(0)
        ...
        fragments = self.meshRas(mesh, cameras=cameras)
        depth_image = fragments.zbuf[..., 0]
        depth_image = 1. / depth_image
        depth_image=torch.where(depth_image==-1.,torch.tensor(0.).to(depth_image.device),depth_image)
