...
parser.add_argument('--dataset',
                    choices=['h36m-p1', 'h36m-p2', 'h36m-p2-mosh', 'lsp', '3dpw', 'mpi-inf-3dhp', '3doh50k',
                             'spec-mtp-cam','3dpw-cam','spec-syn-cam','humman-cam','agora-cam','h36m-cam'],
...
def run_evaluation(model, dataset):
    ...
    #                    gender='female',
    #                    create_transl=False).to(device)
    # smpl_orig=SMPLorig(path_config.SMPL_MODEL_DIR,
    #                     create_transl=False).to(device)
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        gt_pose = batch['global_pose'].to(device)
        ...
        full_images = batch['full_img'].to(device)
        gt_cam_rotmat=batch['cam_rotmat'].to(device)
        gender = batch['gender'].to(device)
        meta_mask = batch['meta_mask'].to(device)
        center = batch['center'].to(device)
        scale = batch['scale'].to(device)
        bbox_height = batch['bbox_height'].to(device)
        orig_shape = batch['orig_shape'].to(device)
        ...
        with torch.no_grad():
            if args.regressor == 'hmr':
                ...
                preds_dict, _ = model(images, meta_mask, center, scale, bbox_height, orig_shape, bbox_info,
                                      is_train=False, J_regressor=None,cam_rotmat=gt_cam_rotmat,full_x=full_images)
                # pred_cam_rotmat = preds_dict['cam_rotmat']
                # pred_cam_rotmat_inverse = torch.inverse(pred_cam_rotmat)
                ...
                pred_vertices = preds_dict['global_output']['global_verts']
        ...
        if eval_pose:
            ...
            gt_vertices = smpl_neutral(global_orient=gt_pose[:, :3], body_pose=gt_pose[:, 3:], betas=gt_betas).vertices
            gt_keypoints_3d = torch.matmul(J_regressor_batch, gt_vertices)
            # gt_keypoints_3d = smpl_orig(global_orient=gt_pose[:, :3], body_pose=gt_pose[:, 3:], betas=gt_betas).joints[:,:24]
        ...
        #                  os.path.join('./notebooks/output/demo_results', dataset_name, args.checkpoint.split('/')[-3]),
        #                  args)
...
if __name__ == '__main__':
    ...
    if args.regressor == 'pymaf_net':
        model = whmr_net(path_config.SMPL_MEAN_PARAMS, pretrained=False)
    ...
    # dataset = Eval_Img_Dataset(args, args.dataset, is_train=False)
    # dataset=EvalBaseDataset(args, args.dataset, is_train=False)
    # Run evaluation
