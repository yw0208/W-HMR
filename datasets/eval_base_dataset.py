...
class EvalBaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False, use_augmentation=True, is_train=True):
        ...
        self.img_dir = '/opt/data/private/datasets/3dpw_img_feature'
    ...
    def __getitem__(self, index):
        ...
        imgname=imgname.split('.')[0]+'.pt'
        imgname_list=imgname.split('/')
        imgname_list[-2]=imgname_list[-2]+'_0'
        imgname='/'.join(imgname_list)
        try:
            img = joblib.load(imgname)
            img= img['s_feat']
            orig_shape = np.array([224,224])
        except:
